...
public class Enemy : MonoBehaviourPun
{
    ...
    void Update()
    {
        ...
        DetectPlayer();
        Debug.Log("Current Entity:" + PhotonNetwork.PlayerList.Length);
    }
    ...
    void DetectPlayer()
    {
        if (Time.time - lastPlayerDetectTime > playerDetectRate)
        {
            ...
            foreach (PlayerController player in GameManager.instance.players)
            {
                ...
                Debug.Log("This is on line 76 and the player.transform.position is" + player.transform.position);

                if (player == targetPlayer)
                {
                    if (dist > chaseRange)
                        targetPlayer = null;
                    Debug.Log("This is on line 81");
                }
                else if (dist < chaseRange)
                {
                    if (targetPlayer == null)
                        targetPlayer = player;
                    Debug.Log("This is on line 87");
                }
            }
        }
    }
